...
export default function QRCodesPage() {
    ...
    const [qrCodes, setQrCodes] = useState([
        { id: "VIP-0042", name: "David G.", event: "Gala Dinner", status: "Active", scans: "1/1", created: "Oct 24, 2025" },
        { id: "STD-1092", name: "Sarah T.", event: "Tech Conference", status: "Used", scans: "1/1", created: "Oct 24, 2025" },
        { id: "STAFF-02", name: "Mike R.", event: "Warehouse", status: "Active", scans: "12/unlimited", created: "Sep 15, 2025" },
        { id: "EMP-049", name: "Elena V.", event: "Headquarters", status: "Inactive", scans: "0/unlimited", created: "Aug 02, 2025" },
        { id: "GUEST-88", name: "Tom B.", event: "Tech Conference", status: "Active", scans: "0/1", created: "Oct 25, 2025" },
    ]);
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <tbody className="divide-y-2 divide-slate-300 text-slate-700 text-sm">
                            {qrCodes.map((qr) => (
                                <tr key={qr.id} className="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4 text-slate-500">{qr.scans}</td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${qr.status === 'Active' ? 'bg-emerald-100 text-emerald-700' :
                                            qr.status === 'Used' ? 'bg-blue-100 text-blue-700' :
                                                'bg-slate-100 text-slate-600'
                                            }`}>
                                            {qr.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 text-slate-500">{qr.created}</td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <button className="p-1.5 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-colors" title="Download QR">
                ...
                <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        <button className="px-3 py-1.5 border border-slate-200 text-slate-600 text-sm font-medium rounded-lg hover:bg-slate-50 hover:text-slate-900 disabled:opacity-50 disabled:cursor-not-allowed" disabled>Previous</button>
                        <button className="px-3 py-1.5 border border-slate-200 text-slate-600 text-sm font-medium rounded-lg hover:bg-slate-50 hover:text-slate-900">Next</button>
                    </div>
                </div>
            </div>
        </div>
